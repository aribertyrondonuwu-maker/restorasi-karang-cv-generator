...
Untuk Tim Pelaksana Swakelola Restorasi Terumbu Karang PPS Bitung TA 2026
Standar Font Min. 12pt | Kop Surat FPIK UNSRAT Resmi | Klausul SK Dekan
"""
...
# Tab Navigasi Utama
tab1, tab2 = st.tabs(["📄 Generator CV Modern (Min. 12pt)", "✉️ Generator Surat Tugas Dinas Luar"])
...
with tab1:
    ...
    with st.form("cv_form", clear_on_submit=False):
        ...
        with col1:
            cv_nama = st.text_input("Nama Lengkap (dengan gelar)", value="Dr. Ir. Ari Berty Rondonuwu, M.Si.", key="cv_nama")
            cv_nip = st.text_input("NIP", value="196801291993031001", key="cv_nip")
            cv_ttl = st.text_input("Tempat, Tanggal Lahir", value="Tareran, 29 Januari 1968", key="cv_ttl")
        with col2:
            cv_jabatan = st.text_input("Jabatan Fungsional", value="Lektor (Penata)", key="cv_jabatan")
            cv_nidn = st.text_input("NIDN", value="0029016804", key="cv_nidn")
            cv_afiliasi = st.text_input("Afiliasi Institusi", value="FPIK Universitas Sam Ratulangi", key="cv_afiliasi")
        
        col3, col4 = st.columns(2)
        with col3:
            cv_alamat = st.text_area("Alamat Rumah", value="Perumahan Duta Graha Blok B No. 14A, Manado-95136", height=60, key="cv_alamat")
            cv_email = st.text_input("Email Aktif", value="arirondonuwu@unsrat.ac.id", key="cv_email")
        with col4:
            cv_telepon = st.text_input("No. Telepon/HP", value="081356033368", key="cv_telepon")
            cv_foto = st.file_uploader("Unggah Pas Foto (3x4)", type=["jpg", "png", "jpeg"], key="cv_foto")
...
with tab2:
    ...
    with st.form("st_form", clear_on_submit=False):
        ...
        with col3:
            tgl_mulai = st.date_input("Tanggal Mulai", value=date(2026, 9, 15), key="st_tgl_mulai")
        with col4:
            tgl_selesai = st.date_input("Tanggal Selesai", value=date(2026, 9, 16), key="st_tgl_selesai")
        with col5:
            jml_hari = st.number_input("Jumlah Hari", min_value=1, max_value=30, value=2, key="st_jml_hari")
        
        # --- NOMOR SURAT ---
        st.subheader("📋 Nomor Surat")
        st_nomor = st.text_input("Nomor Surat", value="800/.../FPIK-UNSRAT/2026", key="st_nomor")
